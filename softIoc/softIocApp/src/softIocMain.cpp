...
void usage(const char *arg0, const std::string &base_dbd) {
  std::cout << "Usage: " << arg0
            << " [-D softIoc.dbd] [-h] [-S] [-s] [-v] [-a ascf]\n"
               ...
               "[-x prefix] [st.cmd]\n"
               "\n"
               "    -D <dbd>  If used, must come first. Specify the path to "
               "the softIoc.dbdfile."
               "        The compile-time install location is saved in the "
               "binary as a default.\n"
               "\n"
               ...
               "\n"
               "    -s  Previously caused a shell to be started.  Now accepted "
               "and ignored.\n"
               "\n"
               "    -v  Verbose, display steps taken during startup.\n"
               "\n"
               "    -a <acf>  Access Security configuration file.  Macro "
               "substitution is\n"
               "        performed.\n"
               "\n"
               "    -m <MAC>=<value>,... Set/replace macro definitions used by "
               "subsequent -d and\n"
               "        -a.\n"
               "\n"
               "    -d <db>  Load records from file (dbLoadRecords).  Macro "
               "substitution is\n"
               "        performed.\n"
               "\n"
               "    -x <prefix>  Load softIocExit.db.  Provides a record "
               "\"<prefix>:exit\".\n"
               "        Put 0 to exit with success, or non-zero to exit with "
               "an error.\n"
               "\n"
               "Any number of -m and -d arguments can be interspersed; the "
               "macros are applied\n"
               "to the following .db files.  Each later -m option causes "
               "earlier macros to be\n"
               "discarded.\n"
               "\n"
               "A st.cmd file is optional.  If any databases were loaded the "
               "st.cmd file will\n"
               "be run *after* iocInit.  To perform iocsh commands before "
               "iocInit, all database\n"
               "loading must be performed by the script itself, or by the user "
               "from the\n"
...
int main(int argc, char *argv[]) {
  try {
    std::string dbd_file(DBD_FILE_REL),
    ...
    while ((opt = getopt(argc, argv, "ha:D:d:m:Ssx:v")) != -1) {
      ...
      case 'x':
        ...
        xmacro += optarg;
        // if (verbose) {
        //     std::cout<<"dbLoadRecords(\""<<exit_file<<"\",
        //     \""<<xmacro<<"\")\n";
        // }
    ...
    if (optind < argc) {
      ...
      ranScript =
          true; /* Assume the script has done any necessary initialization */
    }
